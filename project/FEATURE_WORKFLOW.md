...
This writes the `restart_requested.{env}` flag file (the suffix comes from `MARCEL_ENV` — `prod` or `dev`). The matching host-side systemd path unit (`marcel-redeploy.path` for prod, `marcel-dev-redeploy.path` for dev) watches the flag and triggers `redeploy.sh --env {env}`, which clears the flag, rebuilds the image, and recreates the container. Prod's in-container watchdog (PID 1) then polls `/health` and rolls back via `git revert HEAD` on failure; dev has no watchdog by design, so dev has no automatic rollback. Marcel does **not** restart itself from inside the container. See [docs/self-modification.md](../docs/self-modification.md) for full details.

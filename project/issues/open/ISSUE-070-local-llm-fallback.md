...
- [ ] **070-b** — Add a `LOCAL_MODELS: dict[str, str]` registry in [src/marcel_core/harness/agent.py](src/marcel_core/harness/agent.py) alongside `ANTHROPIC_MODELS` / `OPENAI_MODELS`. Include it in `all_models()` only when `settings.marcel_local_llm_url` is set, so the UI doesn't show broken options.
- [ ] **070-c** — Extend `_resolve_model_string()` in [src/marcel_core/harness/agent.py:58](src/marcel_core/harness/agent.py#L58) to handle the `local:` prefix and/or local model names. Return an `OpenAIModel` instance (not a string) for the local branch, using `OpenAIProvider(base_url=settings.marcel_local_llm_url, api_key='ollama')`. Update the return type to `str | OpenAIModel`. Raise a clear `RuntimeError` if `local:` is requested but `marcel_local_llm_url` is not set.
...
- [ ] **070-h** — Unit test in [tests/harness/test_agent.py](tests/harness/test_agent.py) (create if missing): `_resolve_model_string` returns the right thing for local prefix, raises when URL unset, falls back correctly when non-local model is requested.

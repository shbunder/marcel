...
The current implementation (`subprocess` + `asyncio.wait_for`) only handles the one-shot case: fire, wait, collect output. It cannot:
...
Upgrading to stream-json solves this. The tool becomes an event loop that:
1. Streams events from Claude Code stdout
2. On normal output events — accumulates and forwards to Marcel's response stream (`notify`)
3. On question / permission events — pauses, relays to user, waits for answer, writes answer to Claude Code stdin
4. On completion — returns final result

The MARCEL.md update (committed alongside ISSUE-035 investigation) already documents the intended behavior; this issue makes it real.

...
**Action**: Centralize config to data root and wire up system prompt
**Files Created**:
- `src/marcel_core/defaults/__init__.py` — seed_defaults() copies bundled MARCEL.md and skills to data root on first startup
- `src/marcel_core/defaults/MARCEL.md` — bundled default with response delivery guidance
- `src/marcel_core/defaults/skills/` — bundled default skill docs (banking, browser, developer, docker, icloud, jobs, memory, settings)
**Files Modified**:
- `src/marcel_core/agent/marcelmd.py` — removed project path lookup, only loads from data_root
- `src/marcel_core/skills/loader.py` — removed project path lookup, only loads from data_root/skills/
- `src/marcel_core/harness/context.py` — wired up load_marcelmd_files() and load_skills() into build_instructions_async()
- `src/marcel_core/main.py` — added seed_defaults() call in lifespan
- `src/marcel_core/skills/install_skills.py` — updated to use seed_defaults()
- `CLAUDE.md` — updated skill/MARCEL.md location references
- `project/CLAUDE.md` — updated integration pattern references
- `project/issues/CLAUDE.md` — updated pre-close checklist reference
- `.gitignore` — added .marcel/
**Files Removed**:
- `.marcel/MARCEL.md` — moved to data root
- `.marcel/skills/` — moved to data root (13 files across 8 skill directories)
**Commands Run**: `.venv/bin/pytest tests/ -x -q` → 682 passed

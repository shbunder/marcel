...
1. **Grep for stale references** to anything you changed — renamed symbols, removed flags, new emoji, changed commit format, new config keys:
   ```bash
   grep -rn "<key term>" docs/ ~/.marcel/ .claude/ README.md SETUP.md mkdocs.yml
   ```
   `~/.marcel/` is the zoo checkout — it contains every skill, integration, channel habitat, plus `MARCEL.md` and `routing.yaml`. A rename in marcel-core's plugin API almost always has echoes there.
...
| "`docs/` wasn't touched, so I skipped the grep" | The grep scope is wider than `docs/`: the zoo checkout at `~/.marcel/`, `.claude/`, README, SETUP, mkdocs. Running it is cheap; skipping it is how stale references survive. |

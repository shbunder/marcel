...
- **Skills load from two roots** — `<MARCEL_ZOO_DIR>/skills/` (habitats from marcel-zoo when `MARCEL_ZOO_DIR` is set) and `<MARCEL_DATA_DIR>/skills/` (user customizations; data root wins on collision). The kernel no longer ships any bundled default skills — all live in marcel-zoo. Each skill has `SKILL.md` and optionally `SETUP.md`; SKILL.md may declare `requires:` (inline), `depends_on: [<integration>]` (resolved against `<MARCEL_ZOO_DIR>/integrations/<name>/integration.yaml`), or neither (standalone — always loaded, no `SETUP.md` fallback). Integration handlers use `@register("name.action")` in `<MARCEL_ZOO_DIR>/integrations/<name>/__init__.py`.

...
- **Skills load from two roots** — `<MARCEL_ZOO_DIR>/skills/` (habitats from marcel-zoo when `MARCEL_ZOO_DIR` is set) and `<MARCEL_DATA_DIR>/skills/` (user customizations; data root wins on collision). First-party kernel defaults are seeded from `src/marcel_core/defaults/skills/` to the data root on first start. Each skill has `SKILL.md` and optionally `SETUP.md`; SKILL.md may declare `requires:` (inline) or `depends_on: [<integration>]` (resolved against `<MARCEL_ZOO_DIR>/integrations/<name>/integration.yaml`). Integration handlers use `@register("name.action")` in `src/marcel_core/skills/integrations/` (kernel) or `<MARCEL_ZOO_DIR>/integrations/<name>/__init__.py` (zoo).
